...
public class Pedido implements Comparable<Pedido>{
	...
	public double valorFinal() {
		...
		for (ItemDePedido itemDePedido : itens) {
			if(this.formaDePagamento == 1) {
				valorPedido = (itemDePedido.getPrecoVenda() *DESCONTO_PG_A_VISTA ) + itemDePedido.getPrecoVenda(); 
			}
    ...
    public double valorCatalogoAtual() {
		...
		for (ItemDePedido itemDePedido : itens) {
			soma+=itemDePedido.getPrecoVenda();
		}
    ...
    public double indiceEconomia() {
        return valorFinal() - valorCatalogoAtual(); //Aparentemente acho que está correto
    }
